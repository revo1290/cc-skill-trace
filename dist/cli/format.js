...
export function buildStats(events) {
    ...
    for (const ev of events) {
        ...
        if (ev.source === "claude")
            map[ev.skillName].auto++;
        else
            map[ev.skillName].byUser++;
    }
...
export function renderDashboard(events) {
    ...
    // ── header ──
    lines.push(hr("═"));
    lines.push(chalk.bold.white("  🔍 cc-skill-trace ") +
    ...
    lines.push(chalk.gray("  🤖 Auto-trigger  ") +
...
// ─── Compact list ─────────────────────────────────────────────────────────────
export function renderCompact(events) {
    const lines = [];
